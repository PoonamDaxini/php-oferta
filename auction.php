...
$dbhandle = connect('localhost', 'root', 'root123');
//var_dump($dbhandle);
...
function connect($host, $username, $password) {
    $tries = 0;
    if (!($handle = mysqli_connect("{$host}", $username, $password, "oferta")) && $tries++ < 3) {
...
function get_player($handle) {
    $sql = 'SELECT * FROM users where is_captain != 1 AND bid_value = 0 ORDER BY RAND() LIMIT 1';
    return mysql_fire_query($handle, $sql);
}
